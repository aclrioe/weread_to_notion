 ...
 * 微信读书登录监控脚本 - 响应体版本
 * 
 * 功能：监控微信读书登录API的响应，提取skey和vid信息
 * 
 * 使用方法：
 * 1. 在Quantumult X中添加以下重写规则：
 *    ^https://i\.weread\.qq\.com/login url script-response-body weread_login_monitor.js
 * 
 * 2. 配置环境变量：
 ...
 * 
 * 3. 在微信读书中进行登录操作，脚本会自动捕获并处理响应
 * 
 * 输出：
 * - 提取的skey和vid信息
 * - 推送到指定的GitHub Gist（文件名：weread_login_info.json）
 * 
 * @author: WeRead Monitor
 * @version: 2.0.0
 * @date: 2024
 */
// 创建Env环境实例
...
!(async () => {
  await processResponse($response); // 处理响应
})()
...
/**
 * 处理微信读书登录响应
 * @param {Object} response - 捕获的HTTP响应对象
...
async function processResponse(response) {
  try {
    ...
    // 解析响应体
    let responseData = parseResponseBody(response.body);
    
    // 提取关键信息
    const skey = responseData.skey || "";
    const vid = responseData.vid || "";
    ...
    // 发送通知
    $.msg(NOTIFY_TITLE, NOTIFY_SUCCESS_MSG, `VID: ${vid}, SKEY: ${skey}`);
      ...
      if (gistsResult.status === 200) {
        ...
        if (Array.isArray(gists)) {
          ...
          if (needCreateNew && gistId) {
            ...
            if (debugMode) {
              $.log(
                `未找到包含文件 ${gistFilename} 的Gist，将使用指定的Gist ID: ${targetGistId}`
              );
            }
          }
        }
    ...
    if (!needCreateNew && targetGistId) {
      try {
        ...
        if (updateResult.status === 200) {
          ...
          return {
            success: false,
            message: `Gist更新失败: ${updateResult.status}`,
          };
        }
      } catch (error) {
        $.log(`更新Gist时出错: ${error}`);
